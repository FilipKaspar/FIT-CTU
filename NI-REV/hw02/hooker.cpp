...
int main() {
    ...
    cout << "Modified malloc:" << endl;
    MallocDebug_Done();
    MallocDebug_Init();
    MallocDebug_Init();
    cout << records_head << endl;
    ...
    int* g = (int*) realloc(nullptr, 10 * sizeof(int));

    free(b);
    free(g);
    // free(b); // It's gonna cause double free and program crashes
    // free(e); // Pointer hasn't been allocated yet -> program crashes

    MallocDebug_Done(); // *c and *d hasn't been freed
    MallocDebug_Done();

    cout << "\nAgain standard malloc: " << endl;
    ...
    Sleep(100000); // Added just to see the output
